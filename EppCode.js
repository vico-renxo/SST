...
// Devuelve el DNI (col B) del usuario administrador activo en sesión GAS
function _resolveUsuarioDni() {
  try {
    const email = Session.getActiveUser().getEmail();
    if (!email) return '';
    const hoja = getSpreadsheetPersonal().getSheetByName('PERSONAL');
    ...
    for (let i = 0; i < data.length; i++) {
      const emailFila = (data[i][12] || '').toString().trim().toLowerCase();
      if (emailFila === email.toLowerCase()) {
        return String(data[i][1] || '').trim(); // Col B = DNI
      }
    }
    return '';
  } catch (e) {
...
function execCart(payload){
  try{
    ...
    // 🔔 Notificar al admin que realizó la asignación (resumen único, no por ítem)
    try {
      const adminDni = _resolveUsuarioDni();
      if (adminDni && adminDni !== dni) { // no notificar si el admin se asigna a sí mismo
        const totalEntregas = items.filter(it => String(it.tipo||'').toLowerCase().startsWith('entreg')).length;
        const totalDevoluciones = items.filter(it => String(it.tipo||'').toLowerCase().startsWith('devol')).length;
        let resumen = '';
        if (totalEntregas > 0) resumen += totalEntregas + ' EPP(s) asignado(s)';
        if (totalDevoluciones > 0) resumen += (resumen ? ' · ' : '') + totalDevoluciones + ' devolución(es)';
        enviarPushNotification(
          adminDni,
          'EPP Registrado',
          resumen + ' a ' + nombres + '. Pendiente de confirmación.',
          'epp-admin-asignacion'
        );
      }
    } catch(e) { Logger.log('Push admin error: ' + e); }
...
function confirmarEntregaEpp(idReg, firmaDataUrl){
  ...
  try{
    ...
    // 🔔 Notificar al admin que registró la entrega
    try {
      const usuarioReg = _str(regData[IDX.REG.USUARIO-1]).trim().toLowerCase();
      ...
      for (let p = 0; p < pData.length; p++) {
        if ((pData[p][2]||'').toString().trim().toLowerCase() === usuarioReg) {
