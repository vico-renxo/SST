...
function getCheckListCached_v2() {
  ...
  if (lastRow < 2) {
    ...
    const allData = hoja.getRange(1, 1, lastRow, 6).getDisplayValues();
    checkListCache_v2 = {
      headers: allData[0],
...
function saveDataCheck_v2(obj) {
  try {
    ...
    if (obj.imageData && obj.imageData.includes('base64')) {
      ...
      imageUrl = ''; // En evaluaciones de desempeño no se hereda foto anterior
    }
    ...
    // 3. DEFINIR ESTADO - Calcular porcentaje de cumplimiento
    const respuestas = String(obj.checked).split(',');
    const siCount = respuestas.filter(r => ['si','5','4'].includes(r.trim().toLowerCase())).length;
    const noCount = respuestas.filter(r => ['no','1','2'].includes(r.trim().toLowerCase())).length;
    const totalResp = siCount + noCount;
    const pct = totalResp > 0 ? Math.round((siCount / totalResp) * 100) : 100;
    const status = "Evaluado";
    ...
    // 5. ENVÍO DE ALERTA (cuando cumplimiento < 70%)
    if (pct < 70) {
      sendChecklistEmail_v2(obj, timestamp, imageUrl, status, pct);
    }
...
function sendChecklistEmail_v2(obj, newValue, imageUrl, status, pct) {
  ...
  const subject = "⚠️ Evaluación de Desempeño: " + obj.ad9 + " (" + obj.ad3 + ") - " + status;
  ...
  const body = `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width:600px; margin:auto; background:#ffffff; border:1px solid #e0e0e0; border-radius:8px; overflow:hidden;">
      ...
      <div style="background:${status === 'En Proceso' ? '#d9534f' : '#28a745'}; color:white; padding:15px; text-align:center;">
      ...
      <div style="padding:20px;">
        <table style="width:100%; font-size:13px; color:#444; margin-bottom:20px;">
          <tr><td style="font-weight:bold; width:30%;">Herramienta:</td><td>${obj.ad3}</td></tr>
          <tr><td style="font-weight:bold;">Tipo de Observación:</td><td>${obj.ad4}</td></tr>
          <tr><td style="font-weight:bold;">Evaluador:</td><td>${obj.ad6}</td></tr>
          <tr><td style="font-weight:bold;">Personal Evaluado:</td><td>${obj.ad9}</td></tr>
          <tr><td style="font-weight:bold;">Área:</td><td>${obj.ad2}</td></tr>
          <tr><td style="font-weight:bold;">Ubicación:</td><td>${obj.ad5}</td></tr>
          <tr><td style="font-weight:bold;">Actividad:</td><td>${obj.actividad}</td></tr>
          <tr><td style="font-weight:bold;">Cumplimiento:</td><td>${pct}%</td></tr>
        </table>
...
function getAnalisisIA_v2(id) {
  try {
    const sheet = getCachedSheet_v2('B DATOS');
    const finder = sheet.getRange("A:A").createTextFinder(String(id)).matchEntireCell(true);
    const cell = finder.findNext();
    if (!cell) return { error: "No se encontro la evaluacion solicitada." };

    const rowIndex = cell.getRow();
    const row = sheet.getRange(rowIndex, 1, 1, 16).getDisplayValues()[0];
    const cachedJson = String(row[14] || "").trim();
    if (cachedJson && cachedJson.startsWith("{")) {
      try {
        const parsedCached = JSON.parse(cachedJson);
        if (parsedCached && (parsedCached.fortalezas || parsedCached.oportunidades || parsedCached.objetivos)) {
          return parsedCached;
        }
      } catch (cacheError) {
        Logger.log("Cache IA invalido en O para ID " + id + ": " + cacheError.message);
      }
    }

    const checklistName = String(row[2] || '').trim();
    const colaborador = String(row[5] || '').trim();
    const area = String(row[4] || '').trim();
    const respuestas = String(row[15] || '').split(',').map(v => String(v).trim()).filter(Boolean);
    const items = getCheckListCached_v2().data.filter(r => String(r[1] || '').trim() === checklistName);
    if (!respuestas.length) return { error: "La evaluacion no tiene respuestas para analizar." };

    const escala = respuestas.map(v => parseInt(v, 10)).filter(n => !isNaN(n) && n >= 1 && n <= 5);
    const promedio = escala.length ? (escala.reduce((a, b) => a + b, 0) / escala.length) : 0;
    ...
    respuestas.forEach((valor, idx) => {
      ...
      const categoria = String(item[2] || 'General').trim();
      const pregunta = String(item[3] || `Pregunta ${idx + 1}`).trim();

      if (!categorias[categoria]) categorias[categoria] = [];
      if (!isNaN(score)) categorias[categoria].push(score);
      if (!isNaN(score) && score >= 4) fortalezasItems.push(pregunta);
      if (!isNaN(score) && score <= 3) oportunidadesItems.push(pregunta);
    });

    const categoriasResumen = Object.keys(categorias).map(nombre => {
      const vals = categorias[nombre];
      const avg = vals.length ? vals.reduce((a, b) => a + b, 0) / vals.length : 0;
      return `${nombre}: ${avg.toFixed(1)}/5`;
    }).join(', ');

    const fallback = {
      nombre: colaborador,
      fortalezas: fortalezasItems.length
        ? `${colaborador || 'El colaborador'} muestra mejor desempeno en ${fortalezasItems.slice(0, 3).join(', ')}. El promedio general actual es ${promedio.toFixed(1)} de 5.`
        : `${colaborador || 'El colaborador'} mantiene un desempeno estable y conviene reforzar sus resultados con seguimiento del evaluador.`,
      oportunidades: oportunidadesItems.length
        ? `Se recomienda trabajar especialmente ${oportunidadesItems.slice(0, 3).join(', ')} para elevar la consistencia del desempeno en ${area || 'su area'}.`
        : `No se identificaron brechas criticas; el siguiente paso es sostener el nivel alcanzado con acompanamiento periodico.`,
      objetivos: oportunidadesItems.length
        ? oportunidadesItems.slice(0, 3).map((item, idx) => `Objetivo ${idx + 1}: mejorar ${item} con una accion concreta y seguimiento en la siguiente evaluacion.`)
        : [
            'Mantener el nivel de desempeno actual con seguimiento del evaluador.',
            'Compartir buenas practicas con el equipo para sostener los resultados.',
            'Definir una meta de mejora continua para el siguiente periodo.'
          ]
    };

    const apiKey =
      (typeof API_KEY !== 'undefined' && String(API_KEY).trim()) ||
      (typeof API_KEY_V2 !== 'undefined' && String(API_KEY_V2).trim()) ||
      '';
    if (!apiKey) {
      fallback.cacheTimestamp = new Date().toISOString();
      sheet.getRange(rowIndex, 15).setValue(JSON.stringify(fallback));
      return fallback;
    }

    const detallePreguntas = respuestas.map((valor, idx) => {
      const item = items[idx] || [];
      return `- ${String(item[2] || 'General').trim()} | ${String(item[3] || `Pregunta ${idx + 1}`).trim()} | puntaje: ${valor}`;
    }).join('\n');

    const prompt = `
Eres un asistente interno de desempeÃ±o laboral.
Analiza la siguiente evaluacion y responde SOLO un JSON valido con esta forma:
{"fortalezas":"...","oportunidades":"...","objetivos":["...","...","..."]}

Reglas:
- Escribe en espanol profesional y concreto.
- "fortalezas" debe ser un solo parrafo.
- "oportunidades" debe ser un solo parrafo.
- "objetivos" debe contener exactamente 3 objetivos accionables.
- Basa la respuesta solo en los resultados entregados.

Datos:
Colaborador: ${colaborador || '-'}
Area: ${area || '-'}
Plantilla: ${checklistName || '-'}
Promedio general: ${promedio.toFixed(1)} / 5
Resumen por categoria: ${categoriasResumen || '-'}
...
`;
    ...
    const result = {
      nombre: colaborador,
      fortalezas: parsed.fortalezas || fallback.fortalezas,
      oportunidades: parsed.oportunidades || fallback.oportunidades,
      objetivos: Array.isArray(parsed.objetivos) && parsed.objetivos.length ? parsed.objetivos.slice(0, 3) : fallback.objetivos
    };
    result.cacheTimestamp = new Date().toISOString();
    sheet.getRange(rowIndex, 15).setValue(JSON.stringify(result));
    return result;
  } catch (error) {
    Logger.log("Error en getAnalisisIA_v2: " + error.message);
    return { error: "No se pudo generar el analisis automatico." };
  }
}
...
function updateDataChecklistServer_v2(obj) {
  try {
    ...
    // Col O reservada para el analisis IA cacheado
...
function getCheckListExtraCached() {
  ...
  if (lastRow < 2) {
    ...
    // Leemos 5 columnas
    const values = hoja.getRange(1, 1, lastRow, 6).getValues();
    checkListExtraCache = {
      headers: values[0], 
      data: values.slice(1) 
    };
...
function obtenerItemsPorEquipoExtra(equipo) {
  ...
  return cache.data
    ...
    .map(r => ({
      ...
      opciones: String(r[4] || "").trim(),
      ayuda: String(r[5] || "").trim()
    }));
}
...
function actualizarChecklistPorEquipoExtra(equipo, itemsJson) {
  ...
  let datosDB = [];
  if(lastRow >= 2) {
    datosDB = hoja.getRange(2, 1, lastRow - 1, 6).getValues()
      .map((r, i) => ({
        id: Number(r[0]) || 0,
        eq: String(r[1]).trim(),
        cat: String(r[2]),
  ...
  nuevosItems.forEach(n => {
    if (n.id && existentesPorID.has(n.id)) {
      ...
      if (n.item !== registro.preg || n.categoria !== registro.cat || n.opciones !== registro.opt || (n.ayuda || "") !== registro.ayuda) {
        hoja.getRange(registro.fila, 3, 1, 4).setValues([[n.categoria, n.item, n.opciones, n.ayuda || ""]]);
      }
    }
  });
  ...
  if (nuevos.length > 0) {
    ...
    hoja.getRange(hoja.getLastRow() + 1, 1, filasAInsertar.length, 6).setValues(filasAInsertar);
  }
...
// ================================================
// TARJETA DE DESEMPEÑO: Foto del personal evaluado
// ================================================
function getFotoPersonalEvaluado_v2(nombre) {
  try {
    // Usa la misma instancia cacheada del libro PERSONAL
    const hoja = getSpreadsheetPersonal().getSheetByName('PERSONAL');
    const lastRow = hoja.getLastRow();
    if (lastRow < 2) return { foto: '', cargo: '', empresa: '' };

    // Leer 18 columnas (A-R) para cubrir todas las columnas posibles
    ...
    // Convertir cualquier URL de Drive al formato de imagen directa
    function toDriveImgUrl(url) {
      if (!url) return '';
      const m = url.match(/(?:\/d\/|id=)([a-zA-Z0-9_-]{20,})/);
      return m ? 'https://lh5.googleusercontent.com/d/' + m[1] : url;
    }

    const buscar = norm(nombre);
    if (!buscar) return { foto: '', cargo: '', empresa: '' };

    for (let i = 0; i < data.length; i++) {
      const nombreFila = norm(data[i][2]); // Col C – Nombres
      if (nombreFila === buscar ||
          nombreFila.includes(buscar) ||
          buscar.includes(nombreFila)) {
        const fotoRaw = String(data[i][14] || '').trim(); // Col O – IMAGEN
        return {
          foto:    toDriveImgUrl(fotoRaw),
          cargo:   String(data[i][6]  || '').trim(), // Col G – Cargo
          empresa: String(data[i][4]  || '').trim()  // Col E – Empresa
        };
      }
    }

    Logger.log('getFotoPersonalEvaluado_v2: sin coincidencia para "' + nombre + '"');
    return { foto: '', cargo: '', empresa: '' };

  } catch (e) {
    Logger.log('getFotoPersonalEvaluado_v2 error: ' + e.message);
    return { foto: '', cargo: '', empresa: '' };
  }
}

// ================================================
// TARJETA: Fotos en lote para las tarjetas EV-CARD
// ================================================
function getFotosLotePersonal_v2(nombres) {
  try {
    const hoja = getSpreadsheetPersonal().getSheetByName('PERSONAL');
    const lastRow = hoja.getLastRow();
    if (lastRow < 2 || !nombres || nombres.length === 0) return {};
    ...
    const result = {};
    const pendientes = nombres.map(n => ({ original: n, norm: norm(n) }));

    for (let i = 0; i < data.length; i++) {
      const nombreFila = norm(data[i][2]);
      if (!nombreFila) continue;
      for (let j = 0; j < pendientes.length; j++) {
        const p = pendientes[j];
        if (result[p.original]) continue;
        if (nombreFila === p.norm || nombreFila.includes(p.norm) || p.norm.includes(nombreFila)) {
          const fotoRaw = String(data[i][14] || '').trim();
          if (fotoRaw) result[p.original] = toDriveImgUrl(fotoRaw);
        }
      }
    }
    return result;
  } catch (e) {
    Logger.log('getFotosLotePersonal_v2 error: ' + e.message);
    return {};
  }
}
