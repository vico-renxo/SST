...
function saveFormDataAccidentesV2(req) {
  ...
  // Batch: 7 setValues en una sola llamada en lugar de 7 getRange().setValue() individuales
  if (toAppend.length === 0 && toDelete.length === 0 && lastRowForID) {
    const r0 = lastRowForID.sheetRow;
    // Cols 2-3 (DNI, Descripción)
    d.getRange(r0, 2, 1, 2).setValues([[
      req.name || '',
      req.comment || req.descripcion || ''
    ]]);
    // Cols 7-11 (Responsable, Atención, Detalles, Estado, Timestamp)
    d.getRange(r0, 7, 1, 5).setValues([[
      req.responsable || '',
      req.atencion || req.origen || '',
      req.detalle || '',
      req.estado || '',
      new Date()
    ]]);
