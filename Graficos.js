...
function cleanCache() {
  // Limpiar entradas de datos expiradas (TTL)
  const now = Date.now();
  cacheGraficos.datos.forEach(function(val, key) {
    if ((now - val.timestamp) >= CACHE_DURATION) cacheGraficos.datos.delete(key);
  });
  // Si aun supera límite, desalojar las más antiguas
  if (cacheGraficos.datos.size > cacheGraficos.maxSize) {
    ...
    entries.slice(0, entries.length - Math.floor(cacheGraficos.maxSize / 2))
      .forEach(function(e) { cacheGraficos.datos.delete(e[0]); });
  }
}
...
function getSpreadsheet(tipo) {
  if (!cacheGraficos.spreadsheets.has(tipo)) {
    try {
      ...
      Logger.log(`Error abriendo spreadsheet ${tipo}:`, error);
...
function obtenerDatosOptimizados() {
  ...
  if (cached) {
    Logger.log("Usando datos desde cacheGraficos");
    return cached;
  }

  Logger.log("Cargando datos optimizados...");
  ...
  try {
    ...
    Logger.log(`Datos cargados en ${Date.now() - startTime}ms`);
    return resultado;
    
  } catch (error) {
    Logger.log("Error cargando datos:", error);
    throw error;
  }
}
...
function limpiarCache() {
  ...
  cacheGraficos.spreadsheets.clear(); // liberar referencias a Spreadsheet objects
...
function generarPronosticoConGemini(fechaInicioStr, fechaFinStr) {
  try {
    ...
    Logger.log('Error en generarPronosticoConGemini:', error);
